...
for workload in ${arr[@]}; do
	...
	if ! test -f ./diff_policy/ghost_$workload.out.done; then
		sudo bazel-bin/experiments/scripts/shinjuku.par ghost $workload 30us 10000 201000 10000 agent_shinjuku 15s | tee ./diff_policy/ghost_$workload.out
	...
	if ! test -f ./diff_policy/cfs_$workload.out.done; then
		sudo bazel-bin/experiments/scripts/shinjuku.par cfs $workload 30us 10000 201000 10000 cfs 15s | tee ./diff_policy/cfs_$workload.out
	...
	if ! test -f ./diff_policy/fifo_per_cpu_agent_$workload.out.done; then
		sudo bazel-bin/experiments/scripts/shinjuku.par ghost $workload 30us 10000 201000 10000 fifo_per_cpu_agent 15s | tee ./diff_policy/fifo_per_cpu_agent_$workload.out
	...
	if ! test -f ./diff_policy/agent_sol_$workload.out.done; then
		sudo bazel-bin/experiments/scripts/shinjuku.par ghost $workload 30us 10000 201000 10000 agent_sol 15s | tee ./diff_policy/agent_sol_$workload.out
