...
for workload in ${arr[@]}; do
	...
	if ! test -f ./diff_policy/ghost_$workload.out.done; then
		sudo bazel-bin/experiments/scripts/shinjuku.par ghost $workload 30us 10000 301000 10000 agent_shinjuku 15s | tee ./diff_policy/ghost_$workload.out
	...
	if ! test -f ./diff_policy/agent_cfs_$workload.out.done; then
		sudo bazel-bin/experiments/scripts/shinjuku.par ghost $workload 30us 10000 301000 10000 agent_cfs 15s | tee ./diff_policy/agent_cfs_$workload.out
		mv ./diff_policy/agent_cfs_$workload.out ./diff_policy/agent_cfs_$workload.out.done
		echo "Done agent_cfs_$workload.out"
	fi
